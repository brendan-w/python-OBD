...
class OBD(object):
    ...
    def __load_commands(self):
        ...
        logger.info("finished querying with %d commands supported" %
    ...
    def test_cmd(self, cmd, warn=True):
        ...
        if cmd.mode == 6 and self.interface.protocol_id() not in ["6", "7", "8", "9"]:
            if warn:
                logger.warning(
                    "Mode 06 commands are only supported over CAN protocols")
            return False
