...
def test_raw_string():
    ...
    assert d.raw_string(
        [Message([Frame("A")]), Message([Frame("B")])]) == "A\nB"


def test_pid():
    assert d.pid(m("4100"+"00000000")
                 ).bits == "00000000000000000000000000000000"
    assert d.pid(m("4100"+"F00AA00F")
                 ).bits == "11110000000010101010000000001111"
...
def test_percent_centered():
    ...
    assert float_equals(d.percent_centered(
        m("4100"+"FF")), 99.2 * Unit.percent)
...
def test_current_centered():
    assert d.current_centered(m("4100"+"00000000")) == - \
    ...
    assert float_equals(d.current_centered(
        m("4100"+"0000FFFF")), 128.0 * Unit.milliampere)
...
def test_sensor_voltage_big():
    ...
    assert float_equals(d.sensor_voltage_big(
        m("4100"+"00008000")),   4.0 * Unit.volt)
...
def test_inject_timing():
    ...
    assert float_equals(d.inject_timing(
        m("4100"+"FFFF")),   302 * Unit.degrees)
...
def test_max_maf():
    ...
    assert d.max_maf(m("4100"+"00ABCDEF")) == 0 * Unit.grams_per_second
...
def test_fuel_status():
    assert d.fuel_status(
        m("4100"+"0100")) == ("Open loop due to insufficient engine temperature", "")
    assert d.fuel_status(
        m("4100"+"0800")) == ("Open loop due to system failure", "")
    assert d.fuel_status(m("4100"+"0808")) == ("Open loop due to system failure",
                                               "Open loop due to system failure")
    assert d.fuel_status(m("4100"+"0008")) == ("",
                                               "Open loop due to system failure")
...
def test_obd_compliance():
    ...
    assert d.obd_compliance(
        m("4100"+"21")) == "Heavy Duty Euro OBD Stage VI (HD EOBD-IV)"
    assert d.obd_compliance(m("4100"+"22")) == None


def test_o2_sensors():
    assert d.o2_sensors(m("4100"+"00")) == ((), (False, False,
                                                 False, False), (False, False, False, False))
    assert d.o2_sensors(m("4100"+"01")) == ((), (False, False,
                                                 False, False), (False, False, False, True))
    assert d.o2_sensors(m("4100"+"0F")) == ((),
                                            (False, False, False, False), (True, True, True, True))
    assert d.o2_sensors(m("4100"+"F0")) == ((), (True, True,
                                                 True, True), (False, False, False, False))


def test_o2_sensors_alt():
    assert d.o2_sensors_alt(m("4100"+"00")) == ((), (False, False),
                                                (False, False), (False, False), (False, False))
    assert d.o2_sensors_alt(m("4100"+"01")) == ((), (False, False),
                                                (False, False), (False, False), (False, True))
    assert d.o2_sensors_alt(m("4100"+"0F")) == ((), (False, False),
                                                (False, False), (True, True), (True, True))
    assert d.o2_sensors_alt(m("4100"+"F0")) == ((), (True, True),
                                                (True, True), (False, False), (False, False))
...
def test_monitor():
    ...
    #                [      test      ][      test      ][      test      ]
    v = d.monitor(
        m("41"+"01010A0BB00BB00BB00105100048000000640185240096004BFFFF"))
